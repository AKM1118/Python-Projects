...
for c in cnts:
    ...
    if cv2.contourArea(c) < 500:
        continue
    # pack and order found contour as rectangles
    dot_box_x = []
    ...
    # Remove all corner points which are outside the marker contours
    ...
    # Remove corner points which coincide with the contour corners
    for i in box:
        x1, y1 = i
        for j in dot_box_x:
            x2, y2 = j
            # 6 is a threshold value and can be changed depending on proximity of corner points and contour corners
            if abs(int(x1) - int(x2)) <= 6 and abs(int(y1) - int(y2)) <= 6:
                dot_box_x.remove(j)
                break
    # if contour has lost some of the corner points, ignore it
    if len(dot_box_x) < 12:
        continue
    else:
...
# The same operations as lines 87-156 but for vertical line calculation
for c in cnts_y:
    if cv2.contourArea(c) < 500:
        continue
    dot_box_y = []
...
# Optional distance detection for horizontal (x) and vertical (y) points. Values in lines 206 and 211 should be the dimensions between selected corner points
...
# Save the images with contours/corner points and edged image for testing purposes
