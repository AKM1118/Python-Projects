...
for i in range(len(cntr_box_x) - 1):
    ...
    for j in range(int(len(dot_box_x_1))):
        ...
        # units #mX mY
        D = dist.euclidean((dot_box_x_1[j][0], dot_box_x_1[j][1]), ((dot_box_x_2[j][0]), dot_box_x_2[j][1]))
        ...
        cv2.putText(orig, "{:.4f}px".format(D), (int(dot_box_x_1[j][0]), int(dot_box_x_2[j][1] + 10)),
        ...
        D = dist.euclidean((dot_box_y_1[j][1], dot_box_y_1[j][0]), ((dot_box_y_2[j][1]), dot_box_y_2[j][0]))
        ...
        cv2.putText(orig, "{:.4f}px".format(D), (int(dot_box_y_1[j][1]), int(dot_box_y_2[j][0] + 10)),
