...
# chessboard size for x and y (used in calibration only)
board_x_cal = 10
board_y_cal = 7

# chessboard size for x and y
board_x_detect = 10
board_y_detect = 7
...
board_img_path = '9_boards_20_photos_15/*.jpg'
...
# This function writes calculated angles x,y,z into excel spreadsheet with a defined name
...
# Wrapper function to calculate execution time of a wrapped function
def timeToComplete(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        return val
    return wrapper
# Function to prepare images with multiple chessboards by leaving only one chessboard visible
def divideImage(images):
    ...
    # Draw rectangles over every chessboard except for current board
    for frame in images:
...
# This function takes 3 euler angles and draws axis lines with angle values at the first point of the chessboard
@timeToComplete
def draw(image, corners, imgpts, xAng, yAng, zAng):
 ...
 return img
# Rescales the image to a desired size. For camera images use values around 30
@timeToComplete
...
# Calculates recalibration error
@timeToComplete
...
# This function performs calibration on a set of images then saves calib parameters as a .npz file
@timeToComplete
...
# Calculates euler angles from a provided rotation matrix
@timeToComplete
def getAngles(rvecs):
...
# Finds cornerpoints on a gray image
@timeToComplete
...
def main():
    ...
    # Get calibration parameters. If none present, calculate them from calibration images
    try:
    ...
    # This loop allows to calculate multiple sets of images stored in numerated folders
    for k in range(1,10):
        angle_arr = []
        experiment_img = glob.glob(f'Set_1_{k}/*.jpg')
        i = 1
        # This loop calculates angles for all images in a list, generated by glob function
